...
namespace HCISManagementDashboard
{
    public static class MainModule
    {
        ...
        static internal void GetClientConfig(Stimulsoft.Report.StiReport rpt)
        {
            HCISDataClassesDataContext dc = new HCISDataClassesDataContext();
            var cc = dc.ClientConfigs.FirstOrDefault(c => c.Active == true);
            ...
            if (cc.ReportImage != null)
            {
                using (System.IO.MemoryStream ms = new System.IO.MemoryStream())
                {
                    ...
                    ms.Flush();
                    rpt.Dictionary.Variables.Add("ReportImage", Image.FromStream(ms));
                }
            }
            else
                rpt.Dictionary.Variables.Add("ReportImage", null);

            rpt.Dictionary.Variables.Add("Name", cc == null ? "" : (cc.Name ?? ""));
            rpt.Dictionary.Variables.Add("ReportTitle", cc == null ? "" : (cc.ReportTitle ?? ""));
        }

        #region Persian Date

        public static string today { get { return GetPersianDate(DateTime.Now); } }
        public static string month_start { get { return today.Substring(0, 8) + "01"; } }
        public static string month_name { get { return GetPersianMonthName(DateTime.Now); } }
        public static string year { get { return GetPersianDate(DateTime.Now).Substring(0, 4); } }
        public static int year_num { get { return int.Parse(GetPersianDate(DateTime.Now).Substring(0, 4)); } }

        private static string[] _all_months_names;
        public static string[] all_months_names
        {
            get
            {
                if (_all_months_names == null)
                {
                    _all_months_names = new string[12];
                    for (int i = 0; i < 12; i++)
                    {
                        _all_months_names[i] = GetPersianMonthName(i + 1);
                    }
                }
                return _all_months_names;
            }
        }
