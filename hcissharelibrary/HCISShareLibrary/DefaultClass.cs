...
{
    public class DefaultClass:Client
    {


        protected void SearchInHCIS(List<Person> myHCISdata, Person MyPerson, ref Boolean cancelflag)
        {
            cancelflag = false;
        ...
        public override Person SearchWithMedicalID(string Code, ref Boolean cancelflag)
        {
            ...
            using (HCISDataDataContext dc = new HCISDataDataContext())
            {
                ...
                {
                    SearchInHCIS(myHCISdata, MyPerson, ref cancelflag);
                    if (cancelflag == true)
                        return null;
                    if (myHCISdata.Count == 0)
                        return null;
                ...
                {//Search MEdicalID
                    ...
                    else
                    {
                        ...
                        MyPerson.Address = string.IsNullOrWhiteSpace(NewPerson.HomeCity) ? "" : NewPerson.HomeCity + " " + NewPerson.HomeAddress;
                        dc.SubmitChanges();
                        DeleteSamePersonOfHCIS(MyPerson, ALLPersonINHCIS, dc);
                    }
                    dc.SubmitChanges();
                }
            }
        ...
        private void DeleteSamePersonOfHCIS(Person EditingPerson, List<Person> ALLPersonINHCIS, HCISDataDataContext dc)
        {
            try
            {
